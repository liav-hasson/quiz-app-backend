...
class UserActivityController:
    ...
    def get_user_profile(self, user_id: str) -> Dict[str, Any]:
        ...
        # Get best category
        best_cat_data = self.questions_repository.get_user_best_category(user_id)
        best_category = best_cat_data["category"] if best_cat_data else None
        
        # Get average score (using all-time performance summary)
        perf_data = self.questions_repository.get_user_performance_timeseries(user_id, period="all")
        average_score = perf_data["summary"]["overall_avg"]

        return {
            "XP": user.get("experience", 0),
            "bestCategory": best_category,
            "totalAnswers": user.get("questions_count", 0),
            "averageScore": average_score,
            "lastActivity": user.get("last_activity_date")
        }
